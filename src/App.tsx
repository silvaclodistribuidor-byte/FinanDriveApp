...
function App() {
  ...
  // 1. Metas Diárias - LÓGICA CORRIGIDA (FATURAMENTO BRUTO COMO ALVO)
  const goals = useMemo(() => {
    const now = new Date();
    ...
    // Meta de Contas Diária (Parte Fixa)
    const dailyBillsGoal = totalMonthlyBills / workingDays;
    
    // Meta de Faturamento Bruto (O que o usuário colocou no input)
    // Ex: Salário desejado 13.000 / 26 dias = 500 reais/dia
    const dailyGrossGoalInput = monthlySalaryGoal / workingDays;

    // A meta oficial do dia (O número GRANDE no card)
    // É o valor que o usuário quer girar (R$ 500).
    // PORÉM, se as contas do dia (R$ 600) forem MAIORES que o faturamento desejado,
    // a meta obrigatoriamente sobe para R$ 600 para não fechar no vermelho.
    const dailyTotalGoal = Math.max(dailyGrossGoalInput, dailyBillsGoal);
    
    // O Lucro Projetado (Livre) é a diferença entre a meta total e as contas
    // Ex: Meta 500 - Contas 287 = 213 Livre.
    const dailyProjectedProfit = Math.max(0, dailyTotalGoal - dailyBillsGoal);

    return {
      dailyBillsGoal,      // Custo Fixo
      dailyProjectedProfit, // Lucro (Livre)
      dailyTotalGoal,      // Meta do Dia (O número grande)
  ...
  const stats = useMemo(() => {
    ...
    if (goals.hasGoals) {
      ...
      else if (netToday < goals.dailyTotalGoal) goalExplanation = "Contas pagas. Busque sua meta de faturamento total!";
      else goalExplanation = "Meta total batida hoje! Excelente.";
  ...
  const getShiftGoalText = () => {
    switch (shiftGoalStatus) {
      ...
      case 'between': return 'Contas pagas. Buscando lucro.';
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3 shrink-0">
              ...
              <div className="bg-gradient-to-br from-emerald-900 to-slate-900 rounded-xl p-3 border border-emerald-800/30 shadow-lg col-span-2 md:col-span-2 flex flex-col justify-center items-center relative overflow-hidden">
               ...
               <div className={`${getShiftGoalColor()} rounded-xl p-3 border shadow-lg col-span-2 md:col-span-1 flex flex-col justify-center items-center relative overflow-hidden`}>
                ...
                {/* Visualização detalhada da Meta no Card */}
                <div className="flex gap-2 w-full justify-center">
                   <div className="text-[9px] opacity-80 flex flex-col items-center">
                      <span className="uppercase font-bold">Contas</span>
                      <span>{formatCurrency(goals.dailyBillsGoal, true)}</span>
                   </div>
                   <div className="w-[1px] bg-white/30 h-6"></div>
                   <div className="text-[9px] opacity-80 flex flex-col items-center">
                      <span className="uppercase font-bold">Livre</span>
                      <span>{formatCurrency(goals.dailyProjectedProfit, true)}</span>
                   </div>
                </div>
              </div>
            </div>
                      ...
                      <div className="flex gap-2 mt-2">
                        <div className="bg-indigo-800/50 rounded-lg px-2 py-1 text-xs border border-indigo-400/30">
                         ...
                         <div className="bg-emerald-800/40 rounded-lg px-2 py-1 text-xs border border-emerald-400/30">
                          <span className="opacity-70 block text-[10px] uppercase font-bold text-emerald-200">Lucro Livre</span>
